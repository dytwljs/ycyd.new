...
module.exports = class extends Base {
  ...
  async updateAction() {
    ...
    if (think.isEmpty(newCartInfo)) {
      ...
      if (!think.isEmpty(productInfo.goods_specification_ids)) {
        goodsSepcifition = await this.model('goods_specification').field(['yc_goods_specification.*', 'yc_specification.name']).join('yc_specification ON yc_specification.id=yc_goods_specification.specification_id').where({
          'yc_goods_specification.goods_id': goodsId,
          'yc_goods_specification.id': {'in': productInfo.goods_specification_ids.split('_')}
        }).select();
      }
