        ...
        <div class="content-main">
            <div class="form-table-box">
                <el-form ref="infoForm" :rules="infoRules" :model="infoForm" label-width="120px">
                    ...
                    <el-form-item v-if="infoForm.isRecordNew" label="手机号." prop='mobile'>
                        <el-input   v-model="infoForm.mobile"></el-input>
                        <!-- <el-input type="number" v-model="infoForm.mobile" placeholder="请输入11位手机号码" pattern="\d{11}"></el-input> -->
                    </el-form-item>
                    <el-form-item v-if="!infoForm.isRecordNew" label="手机号">
                      <el-breadcrumb-item  >{{infoForm.mobile}}</el-breadcrumb-item>
                    </el-form-item>
...
export default {
    ...
    onSubmitInfo() {
      ...
      this.$refs["infoForm"].validate(valid => {
        if (!valid) {
          ...
          //取得销售商等级
          var sale = that.parentSale.filter(function(p) {
            // console.log(p);
            if (p.mobile == that.infoForm.parent_id) return p;
          });
          if (sale && sale.length > 0) that.infoForm.layer = sale[0].layer + 1;

          if (that.infoForm.isRecordNew) {
            var userInfo = localStorage.getItem("userInfo");
            // that.infoForm.id=UUID.v1();
            that.infoForm.create_admin = JSON.parse(localStorage.userInfo).id;
            that.infoForm.create_time = moment(new Date()).format(
              "YYYY-MM-DD HH:mm:ss"
            );

            this.axios
              .post("sale/findSaleReg", that.infoForm)
              .then(response => {
                if (response.data.errno === 0) {
                  this.axios
                    .post("sale/storeReg", that.infoForm)
                    .then(response => {
                      if (response.data.errno === 0) {
                        this.$message({
                          type: "success",
            ...
            this.axios.post("sale/storeReg", that.infoForm).then(response => {
              if (response.data.errno === 0) {
                this.$message({
                  type: "success",
                  message: "保存成功"
    ...
    getInfo() {
      ...
      this.axios
        ...
        .then(response => {
          ...
          that.infoForm .authorize  =(resInfo.authorize==1);
  ...
  mounted() {
    this.getParentSale();
    this.infoForm.mobile = this.$route.query.mobile || 0;
    this.getInfo();
  }
};
</script>
